...
#ifndef _mac_vivado_dma32_w32_dataflow_in_loop_bkb_H_
#define _mac_vivado_dma32_w32_dataflow_in_loop_bkb_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "mac_vivado_dma32_w32_dataflow_in_loop_bkb_memcore.h"

SC_MODULE(mac_vivado_dma32_w32_dataflow_in_loop_bkb) {
...
protected:
    ...
    // buffer instances
    mac_vivado_dma32_w32_dataflow_in_loop_bkb_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(mac_vivado_dma32_w32_dataflow_in_loop_bkb) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "mac_vivado_dma32_w32_dataflow_in_loop_bkb_memcore_U_";
            name += szTmp;

            buffer[i] = new mac_vivado_dma32_w32_dataflow_in_loop_bkb_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "mac_vivado_dma32_w32_dataflow_in_loop_bkb.trace");
            // ports
